...
## gas-and-outcome vectors (`components/proposals/<series>/<proposal>/gas/`)

**Specified here.** One invocation of a stated body of bytecode, under a rule set the fixture names
and defines itself, asserting **the gas it consumed, how it ended, and what it handed back**.

**This shape exists because a state fixture cannot publish a number.** A state fixture measuring gas
has to store the measurement and assert a **state root** over it — `sload_cost.json` runs
`GAS SLOAD POP GAS SWAP1 SUB SSTORE` and hashes the result — so the figure is inside a hash, and a
consumer that disagrees learns only that some root moved. A root is also never network-agnostic (see
`components/README.md`), which is why every `state/` fixture stays under `networks/` however
proposal-shaped its subject. This shape asserts the figure directly, and asserts an **outcome** that
no root can express at all: a reverted frame and a frame that halted exceptionally leave the same
state and are different results.

### The unit is a short body, not one opcode

An opcode cannot be executed without operands and the operands cost gas, so the measurable thing is
always a body. Upstream reaches the same conclusion from the other direction:
`ethereum/execution-specs`' `CodeGasMeasure` wraps a fragment in `GAS … GAS SWAP1 SUB` and subtracts
a stated overhead constant.

**This shape subtracts nothing.** It publishes each body's whole cost and pairs vectors that differ
in one place, so the overhead cancels in the **reader's** subtraction rather than in the generator's.
That is deliberate: a subtracted overhead is a derived constant sitting inside an asserted value,
and when it is wrong the fixture is wrong in a way nothing reports.

### The rule set is named LOCALLY, and defined in the same file

The naming problem this shape had to solve is that a gas figure means nothing without saying which
rules produced it, while borrowing a network's upgrade label would make the fixture network-scoped —
`ETC_Phoenix` is not a thing Ethereum has, and `Byzantium` is not a thing this chain's schedule says.

So a file carries a **`ruleSets` map**. Each entry has a name that is local to that file, an ordered
`proposals` journal in the client's own citation form (`EIP-140`, `ECIP-1017`), and a `generatedWith`
block recording how each oracle was configured to reach it. A vector references an entry by name, and
a harness resolves that name **here**, never in a schedule.

**`proposals` is an ordered journal, not a set.** It records what was adopted, in order — a proposal
can be adopted, withdrawn and re-adopted on a live chain, so membership is not the question it
answers. No fixture depends on that distinction today; it is stated so one does not quietly turn the
field into a set.

> #### Where the GENERATOR cannot activate a single proposal, the fixture owes the reader two things
>
> **This is a property of the oracle you chose, not of the world, and the first version of this
> section said the latter.** It read *"no oracle can activate a single proposal in isolation"*, which
> is a claim about every implementation made from two of them — the exact shape of absence claim this
> repository documents elsewhere. It is false. The Parity-lineage chainspec has **per-proposal
> activation keys**: `eip140Transition` is read by `openethereum/openethereum`,
> `openethereum/parity-ethereum` and `NethermindEth/nethermind`, and it is load-bearing rather than
> decorative — OpenEthereum sets `schedule.have_revert = block_number >= self.eip140_transition`
> (`ethcore/types/src/engines/params.rs`) and gates the opcode on it
> (`ethcore/evm/src/interpreter/mod.rs`), and Nethermind sets `releaseSpec.IsEip140Enabled` from the
...
> make is a whole upgrade.** A fixture generated by an oracle of that kind cannot enforce the
> attribution its own filing claims, and it owes the reader two things instead:
>
> 1. **A control across the same step** — a body whose answer is identical at both rule sets, showing
>    the step alone changes nothing.
> 2. **Evidence the bodies cannot reach the other members of that step** — for an EVM subject, that
>    the sibling proposals' opcodes are absent from the bytecode, established by walking the code with
>    PUSH data skipped and the walk calibrated against a body that does contain one.
>
> Without both, the fixture is asserting an upgrade's effect while filed under a proposal's name.

### The schema

```jsonc
{
  "<proposal><Subject>": {                    // e.g. eip140RevertGasAndOutcome
    "_info": { },
    "invocationContext": {                    // the frame every vector runs in
      "depth": "0", "static": false
    },
    "ruleSets": {
      "withoutEip140": {
        "proposals":     [ "EIP-2", "EIP-7" ],
        "generatedWith": { "go-ethereum": "…", "besu": "evmtool --fork=Homestead" }
      },
      "withEip140": { }
    },
    "vectors": [
      { "name": "…", "comment": "…",
        ...
        "gasBudget":  "100000",
        "gasUsed":    "18",
        "outcome":    "reverted",             // stopped | reverted | halted
        "returnData": "0x…",
        "halt":       "OutOfGasError",        // present only when outcome is `halted`
        "tags":       [ "subject" ] }
    ]
  }
}
...
| `gasUsed` | yes | gas the frame consumed |
| `outcome` | yes | `stopped`, `reverted` or `halted` |
| `returnData` | yes | bytes handed to the caller; `"0x"` when none. A `halted` frame always has none |
| `halt` | only when `halted` | the executable specification's own exception class name |
| `tags` | no | free-form; a reader must not depend on one |

**`outcome` is the client's vocabulary and `halt` is the specification's.** The three outcomes are
`org.fukuii.evm.Outcome`'s `Stopped` / `Reverted` / `Halted`, and the halt names are the class names
in `ethereum/execution-specs`' `forks/*/vm/exceptions.py` — `OutOfGasError`, `InvalidOpcode`,
`StackUnderflowError` and the rest. **Neither was invented here**, which is the same rule this file
states for transaction refusals: never author a name no consumer maps.

Note that a revert is *not* a member of the halt vocabulary in either place — the specification makes
`Revert` a sibling of `ExceptionalHalt` rather than a member of it, and the client makes `Reverted` a
sibling of `Halted`. A fixture that reports a revert with a `halt` name is malformed.

### On a `halted` vector, `gasUsed` restates the budget and asserts nothing

An exceptional halt consumes everything, so `gasUsed == gasBudget` on those vectors **by definition**.
What they assert is the `outcome` and the `halt`. A consumer must not read a matching `gasUsed` there
as evidence its gas accounting is right.

The honest way to make that visible is a pair: run one body that halts and one that reverts at two
budgets each. The halted figure follows the budget; the reverted figure does not move. Both pairs are
in the EIP-140 suite for exactly this reason.

### `callValue` is a real input; the caller and callee are not

Measured against both oracles: changing the caller, the callee or the calldata leaves every result
identical, and a **non-zero call value does not** — no account in a component fixture is funded, so a
...
a transaction, and transaction-level refund capping are settled *around* an invocation, not inside
one — so EIP-2028, EIP-2930 and EIP-3529's transaction-level half need a different seam (`t8n`, which
reports a receipt's `gasUsed`) and a shape that is not this one. Do not stretch this schema to cover
them; a fixture that states a `gasBudget` and a `code` and silently means a whole transaction is
asserting something its own fields do not describe.

### `eip140RevertGasAndOutcome` — the failure that keeps its gas

The first suite in this shape, and it was chosen because its subject exercises **both** halves: REVERT
is a failure that returns gas *and* hands back a payload, so a shape asserting only a number or only
an outcome could not express it.

Its `withoutEip140` rule set stops at Homestead rather than Spurious Dragon, and the reason is an
instrument limit rather than a claim: **besu's evmtool has no name for Spurious Dragon**, and
Homestead is the one pre-adoption set both oracles can address. Widening it in go-ethereum leaves all
twelve results byte-identical, which the fixture records.
